...
def compute_snapshot(cfg: dict | None = None) -> dict:
    ...
    btc_candles = daily_candles = four_h_candles = htf_candles = None
    ...
    try:
        ...
        btc_candles = hl_client.candles("BTC", interval=cfg.get("timeframe", "15m"), lookback_hours=48)
        htf_candles = hl_client.candles("BTC", interval=cfg.get("higher_timeframe", "1h"), lookback_hours=200)
        # Separate from btc_candles/htf_candles above (15m/1h, tuned for the
        # live SMC screener) — Section 5's majors rule specifically wants
        # Daily + 4H confirmation, so it gets its own fetch.
        daily_candles = hl_client.candles("BTC", interval="1d", lookback_hours=24 * 220)
        four_h_candles = hl_client.candles("BTC", interval="4h", lookback_hours=24 * 20)
